  ...
  <div>
    <div class="config-item">
      ...
      <button class="folder-select-btn" @click="showModal = true">
        {{ (config?.folderName as string) || '书签栏根目录（默认）' }}
      </button>
    </div>
    <div class="config-item">
      <label>布局（列 × 行）</label>
      ...
      <div v-if="showModal" class="folder-modal-overlay" @click.self="showModal = false">
        <div class="folder-modal">
          ...
          <div class="folder-modal-body">
            <div
              class="folder-node-row root-default-row"
              :class="{ selected: !(config?.folderId) }"
              @click="handleSelect('', '')"
            >
              <span class="folder-arrow invisible">▶</span>
              <span class="folder-icon">🏠</span>
              <span class="folder-title">书签栏根目录（默认）</span>
            </div>
            <FolderNode
